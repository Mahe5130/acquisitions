...
const testConnection = async () => {
  ...
  try {
    ...
    console.log('✅ Database operations working. Test records:', testResult.rows[0].count);
    ...
    if (error.message.includes('ECONNREFUSED')) {
      console.log('💡 Tip: Make sure the neon-local container is running and healthy');
    }
    ...
    if (error.message.includes('SSL')) {
      console.log('💡 Tip: Neon Local uses self-signed certificates. Make sure ssl.rejectUnauthorized is set to false');
    }
