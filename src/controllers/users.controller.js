...
export const updateUserById = async (req, res, next) => {
    try {
        ...
        // Check if user is trying to update their own information or if admin is updating
        if (req.user.id !== id && req.user.role !== 'admin') {
            logger.warn(`Unauthorized update attempt: User ${req.user.email} tried to update user ID ${id}`);
        ...
        // Check if non-admin user is trying to change role
        if (updates.role && req.user.role !== 'admin') {
            logger.warn(`Unauthorized role change attempt: User ${req.user.email} tried to change role`);
            return res.status(403).json({
                error: 'Forbidden',
...
export const deleteUserById = async (req, res, next) => {
    try {
        ...
        if (req.user.id !== id && req.user.role !== 'admin') {
            logger.warn(`Unauthorized delete attempt: User ${req.user.email} tried to delete user ID ${id}`);
            return res.status(403).json({
                error: 'Forbidden',
